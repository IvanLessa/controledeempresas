...
namespace ProjetoAula05.Entities
{
    ...
    public class Funcionario
    {
        ...
        public string? Matricula
        {
            ...
            {
                if (string.IsNullOrEmpty(value))
                    throw new ArgumentException("A Matrícula do funcionário não pode ser vazia.");

                var regex = new Regex("^[0-9]{4,8}$");
                if (!regex.IsMatch(value))
                    throw new ArgumentException("Matrícula do funcionário é inválida. Informe de 4 a 8 dígitos numéricos.");
